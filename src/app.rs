...
impl App {
    ...
    async fn handle_tick(&mut self) -> Result<()> {
        ...
        // Remove completed download channels (reverse to preserve indices)
    ...
    async fn handle_enter(&mut self) -> Result<()> {
        match self.view {
            ...
            View::Downloaded => {
                ...
                if let Some(selected) = self.downloaded_state.selected() {
                    if selected < self.albums.len() {
                        if self.albums[selected].tracks.is_empty() {
                            self.status_msg = "Loading album...".to_string();
                            let url = self.albums[selected].item_url.clone();
                            if let Some(ref client) = self.client {
                                match client.fetch_album_details(&url).await {
                                    Ok(detail) => {
                                        self.albums[selected].tracks = detail.tracks;
                                        self.albums[selected].about = detail.about;
                                        self.albums[selected].credits = detail.credits;
                                        self.albums[selected].release_date = detail.release_date;
                                    }
                                    Err(e) => {
                                        self.status_msg = format!("Failed to load album: {}", e);
                                        return Ok(());
    ...
    async fn download_selected_album(&mut self) {
        let album_idx = match self.view {
            View::Collection => self.collection_state.selected(),
            View::Downloaded => self.downloaded_state.selected(),
            _ => None,
        };
        ...
        // Fetch album details if tracks aren't loaded yet
    ...
    async fn download_all_albums(&mut self) {
        let cookie = self.auth.as_ref().map(|a| a.identity_cookie.clone()).unwrap_or_default();
        let mut count = 0;

        // Fetch details for albums that don't have tracks loaded
